...
Lemonade.FPS = {
	...
	isDelaying: false,
	/**
	 * @desc Called with each frame, to update our times and frame count for the FPS counter.
	 */
	updateFPS: function() {
		this.frameNumber++;
	...
	/**
	 * @desc Get the current FPS from the FPS counter.
	 * @returns {number} Current frames per second read by counter.
	 */
	getFPS: function() {
		return this.fp;
	}
...
/**
 * @desc Timer object. Should eventually turn into a class / function prototype.
 */
Lemonade.Timer = {
	curTime: 0,
	timeInc: 5,
	/**
	 * @desc Get the current time as read by the timer.
	 * @returns {number} Current time tracked by timer.
	 */
	getCurrentTime: function() {
		return this.curTime;
	},
	/**
	 * @desc Increment the current time by the set increment.
	 */
	incTime: function() {
		this.curTime++;
	},
	/**
	 * @desc Set the increment for the timer with each step. Default 5
	 * @param {number} ti - New time increment.
	 */
	...
	/**
	 * @desc Reset the time tracked by the timer.
	 */
	resetTime: function() {
		this.curTime = 0;
	}
...
/**
 * @desc Lovely MD5 Function from https://css-tricks.com/snippets/javascript/javascript-md5/
 * @param {string} string - String that is to be hashed using the MD5 Algorithm.
 */
...
/**
 * @desc Homebrew method of adding includes in HTML. Does this by adding separate <script> elements referencing script name.
 * @param {string} filename - File name that is to be included. Relative to the site.
 * @param {string} name - This is the name to set as the id of the script, to prevent multiple includes of same script.
 */
...
/**
 * @desc Convert integer / number into Hex values.
 * @param {number} c - Number to convert into Hex.
 * @returns {string} Hex value of the number supplied.
 */
...
/**
 * @desc Convert an RGB value into a Hex string. For use with CSS / Canvas
 * @param {number} r - Red value
 * @param {number} g - Green value
 * @param {number} b - Blue Value
 * @returns {string} Hex value for the RGB color supplied.
 */
Lemonade.rgbToHex = function(r, g, b) {
	return "#" + Lemonade.componentToHex(r) + Lemonade.componentToHex(g) + Lemonade.componentToHex(b);
};
/**
 * @desc Get if an array contains the element supplied.
 * @param {object} a - Array to look through
 * @param {object} obj - Element to see if it exists in the array.
 * @returns {boolean} If the element was found in the array
 */
...
/**
 * @desc Get the index of an element in an array.
 * @param {object} a - Array to search through
 * @param {object} obj - Element to find index of in the array.
...
/**
 * @desc Create a clone of an object.
 * @param {object} src - Source object to clone into a new object.
 * @returns {object} Cloned object with same fields.
 */
Lemonade.clone = function(src) {
	return Object.assign({}, src);
	/**
	 * Previous method of cloning an object with ES5.
	 * 
...
// Include our needed quadtree library for collision detection.
Lemonade.include("quadtree.js", "quadtree");
/**
 * @desc Get the contents of a file via a URL. Used to get json data, or other.
 * @param {string} file - Filename that should be read (relative to the main URL)
 * @param {function} callback - Callback that will be called with the response text as argument.
 */
...
/**
 * @desc Alias for the Lemonade.readTextFile function.
 * @param {string} file - Filename that should be read (relative to the main URL)
 * @param {function} callback - Callback that will be called with the response text as argument.
 */
 ...
 * @returnes {number} Distance between the 2 points.
 */
Lemonade.getDistance = function(x1, y1, x2, y2) {
	return Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
};
/**
 * @desc Current version of the Lemonade engine.
 * @constant {number}
 * @namespace Lemonade
 */
Lemonade.version = 1.001;
/**
 * @desc Describes the current state of the engine loop. Set to false, to either pause the logic / render, or stop entirely.
 * @namespace Lemonade
 */
Lemonade.running = true;
/**
 * @desc Whether to auto clear the content on the screen (essentially for buffering to keep from previous draws showing.)
 * @namespace Lemonade
 */
Lemonade.autoClear = true;

/**
 * @desc Whether to show collision rectangles or not (only for entities with Collide component)
 * @namespace Lemonade
 */
Lemonade.showCollisionRects = false;
/**
 * @desc Whether Touch should act as a mouse event, or separate type of event.
 * @namespace Lemonade
 */
Lemonade.countTouchAsMouse = true;
/**
 * @desc If false, will treat all touch events as a single type.
 * @namespace Lemonade
 */
Lemonade.multiTouch = false;

/**
 * @desc Whether to allow the default keypress action (such as F12, or arrow keys)
 * @namespace Lemonade
 */
Lemonade.allowKeyPress = true;

/**
 * @desc Collection of Collision related functionality
 * @namespace Lemonade
 */
Lemonade.Collision = {};
/**
 * @desc Initialize collision detection. Requires QuadTree be loaded. Initializes with the widht / height of Canvas.
 * @namespace Lemonade.Collision
 */
...
/**
 * @desc Point object that can be initialized. 2D and 3D.
 * @namespace Lemonade
 ...
 * @returns {object} New point object with the supplied points.
 */
...
/**
 * @desc Initialize the Lemonade engine. Load necessary components, and set listeners.
 * @namespace Lemonade
 * @param {number} w - Width of the game space (viewport)
 * @param {number} h - Height of the game space (viewport)
 * @param {string} id - ID Attribute of the game canvas element.
 * @param {boolean} autorun - Whether to start the game loop immediately.
 * @param {boolean} webglEnabled - Whether to use WebGL for rendering (2d and 3d)
 * @param {number} debugMode - The debug mode to be used for logging.
 */
...
/**
 * @desc Global object used to call opengl related functions.
 */
var gl; // Used as global object to call opengl related functions.

/**
 * @desc Alias for the Lemonade.Graphics.threed.translateToNormal function.
 * @param {number} num - Number to convert
 * @param {string} woh - Wether to get Width (FLAG_WIDTH) or Height.
...
/**
 * @desc Container for graphics related methods.
 * @namespace Lemonade
 */
Lemonade.Graphics = {
	/**
	 * @desc Graphics drawing type.
	 */
	graphicsType: GRAPHICS_CANVAS,

	/**
	 * @desc Container for 3D related methods / variables.
	 */
	threed: {
		/**
		 * @desc Whether 3D mode should be on or off.
		 */
		enabled: false, // Disable 3d by default

		/**
		 * @desc Convert value from Normal to World / screenpoint.
		 * @param {number} num - Number to convert
		 * @param {string} widthOrHeight - Wether to get Width (FLAG_WIDTH) or Height.
		 * @returns {number} Converted value.
		 */
		...
		/**
		 * @desc Convert value from Screenpoint to World / Normal.
		 * @param {number} num - Number to convert
		 * @param {string} widthOrHeight - Wether to get Width (FLAG_WIDTH) or Height.
		 * @returns {number} Converted value.
		 */
		...
		/**
		 * @desc Container for methods relating to 3d / GL buffers.
		 */
		buffer: {
			// This assumes that the array is of vertices with 3 coordinates
			/**
			 * @desc Convert array of vertices and faces to a buffer object.
			 * @param {object} arrayVert - Array of Vertices.
			 * @param {object} arrayFace - Array of Faces.
			 * @returns {object} Buffer object with converted faces and vertices.
			 */
			...
			/**
			 * @desc Get face buffer from an array of faces.
			 * @param {object} arrayFace - Array of faces.
			 * @returns {object} Face buffer object.
			 */
			...
			/**
			 * @desc Get vertex buffer from an array of vertices.
			 * @param {object} arrayVert - Array of vertices.
		...
		/**
		 * @desc Functions relating to shaders with GL
		 */
		shader: {
			/**
			 * @desc Default vertex source shader.
			 */
			...
			/**
			 * @desc Default fragment source shader.
			 */
			...
			/**
			 * @desc Default Fragment UV Shader.
			 */
			...
			/**
			 * @desc Generate a color using test shader sources.
			 */
			...
			/**
			 * @desc Generate a test UV with test shaders UVs.
			 */
			...
			/**
			 * @desc Get a shader based on source 
			 * @param {string} source - Source that is to be interpreted into shader.
			 * @param {string} type - Type of shader
			 * @param {string} typeString - Type as a string.
			 * @returns {object} Shader object that was created with GL.
			 */
			...
			/**
			 * @desc Bind a shader to the GL context.
			 * @param {object} shader - Shader to bind.
			 */
		...
		/**
		 * @desc Initialize OpenGL with the engine / webgl.
		 */
	...
	/**
	 * @desc Change the graphics type to the one specified.
	 * @param {string} newType - The new graphics type that is to be used with the engine. (At this time, only supports 1)
	 */
	changeGraphicsType: function(newType) {
		this.graphicsType = newType;
	},
	/**
	 * @desc Prepare the engine for use in a 3d context.
	 */
	prepThreeD: function() {
...
/**
 * @desc Methods relating to the game view camera. At this time 1 camera supported. Potentially could freeze and restore though.
 */
Lemonade.Camera = {
	/**
	...
	y: 0,
	/**
	 * @desc Z coordinate of the camera. Primarily for 3d Use.
	 */
	z: 0, // Not used yet.
	/**
	 * @desc Scale X of the camera view.
	 */
	sx: 1,
	/**
	 * @desc Scale Y of the camera View.
	 */
	sy: 1,
	/**
	 * @desc Scale Z of the camera view.
	 */
	sz: 1,
	/**
	 * @desc Get X of the camera.
	 * @returns X coordinate of the camera.
	 */
	getX: function() {
		return this.x;
	},
	/**
	 * @desc Get Y of the camera.
	 * @returns Y coordinate of the camera.
	 */
	getY: function() {
		return this.y;
	},
	/**
	 * @desc Get Z of the camera.
	 * @returns Z coordinate of the camera.
	 */
	getZ: function() {
		return this.z;
	},
	/**
	 * @desc Get X Scale of the camera.
	 * @returns X Scale of the camera.
	 */
	getScaleX: function() {
		return this.sx;
	},
	/**
	 * @desc Get Y Scale of the camera.
	 * @returns Y Scale of the camera.
	 */
	getScaleY: function() {
		return this.sy;
	},
	/**
	 * @desc Get Z Scale of the camera.
	 * @returns Z Scale of the camera.
	 */
	getScaleZ: function() {
		return this.sz;
	},
	/**
	 * @desc Move the camera X position by dx.
	 * @param {number} dx - Amount to move the X coordinate by.
	 * @returns Current X coordinate of the position.
	 */
	moveX: function(dx) {
		this.x += dx;
		return this.x;
	...
	/**
	 * @desc Move the camera Y position by dy.
	 * @param {number} dy - Amount to move the Y coordinate by.
	 * @returns Current Y coordinate of the position.
	 */
	moveY: function(dy) {
		this.y += dy;
		return this.y;
	...
	/**
	 * @desc Move the camera Z position by dz.
	 * @param {number} dz - Amount to move the Z coordinate by.
	 * @returns Current Z coordinate of the position.
	 */
	moveZ: function(dz) {
		this.z += dz;
		return this.z;
	...
	/**
	 * @desc Move the camera X Scale by dsx.
	 * @param {number} dsx - Amount to move the X Scale by.
	 * @returns Current X Scale of the camera.
	 */
	...
	/**
	 * @desc Move the camera Y Scale by dsx.
	 * @param {number} dsy - Amount to move the Y Scale by.
	 * @returns Current Y Scale of the camera.
	 */
	...
	/**
	 * @desc Move the camera Z Scale by dsx.
	 * @param {number} dsz - Amount to move the Z Scale by.
	 * @returns Current Z Scale of the camera.
	 */
	...
	/**
	 * @desc Set the current X coordinate of the camera.
	 * @param {number} nx - New X coordinate.
	 * @returns Current X coordinate after change.
	 */
	setX: function(nx) {
		this.x = nx;
		return this.x;
	...
	/**
	 * @desc Set the current Y coordinate of the camera.
	 * @param {number} ny - New Y coordinate.
	 * @returns Current Y coordinate after change.
	 */
	setY: function(ny) {
		this.y = ny;
		return this.y;
	...
	/**
	 * @desc Set the current Z coordinate of the camera.
	 * @param {number} nz - New Z coordinate.
	 * @returns Current Z coordinate after change.
	 */
	setZ: function(nz) {
		this.z = nz;
		return this.z;
	...
	/**
	 * @desc Set the current X scale of the camera view.
	 * @param {number} nsx - New Scale X
	 * @returns Current X Scale after change.
	 */
	...
	/**
	 * @desc Set the current Y scale of the camera view.
	 * @param {number} nsy - New Scale Y
	 * @returns Current Y Scale after change.
	 */
	...
	/**
	 * @desc Set the current Z scale of the camera view.
	 * @param {number} nsz - New Scale Z
	 * @returns Current Z Scale after change.
	 */
	...
	/**
	 * @desc Move the Camera position by specified values.
	 * @param {number} dx - Amount to move X position.
	 * @param {number} dy - Amount to move Y position.
	 * @param {number} dz - Amount to move Z position.
	 */
	...
	/**
	 * @desc Change the Camera Scale by specified values (all 3)
	 * @param {number} dsx - Amount to move the X scale by.
	 * @param {number} dsy - Amount to move the Y scale by.
	 * @param {number} dsz - Amount to move the Z scale by.
...
/**
 * @desc Canvas related functions and variables.
 */
Lemonade.Canvas = {
	/**
	 * @desc Width of the canvas. Set by the initialize function.
	 */
	canvasWidth: 0,
	/**
	 * @desc Height of the canvas. Set by the initialize function.
	 */
	canvasHeight: 0,
	/**
	 * @desc DOM Object for the canvas element for the engine to use.
	 */
	canvasObject: undefined,
	/**
	 * @desc Context (2d or 3d) for the canvas element.
	 */
	context: undefined,
	/**
	 * @desc X ratio for the Canvas to Real Width
	 */
	xRatio: 1,
	/**
	 * @desc Y ratio for the Canvas to Real Height
	 */
	yRatio: 1,
	/**
	 * @desc Whether the canvas should auto clear after each draw.
	 */
	autoClear: true,
	/**
	 * @desc FPS cap.
	...
	FPS: 30,
	/**
	 * @desc Initialize the engine with a 2d Context.
	 * @param {number} w - Width of the game view.
	 * @param {number} h - Height of the game view.
	 * @param {string} divName - The Id of the Div to use as canvas.
	 * @param {boolean} autoScale - Whether the canvas should auto scale ornaw.
	 */
	initialize2D: function(w, h, divName, autoScale) {
		this.initialize(w, h, divName, autoScale);
	},
	/**
	 * @desc Initialize the engine with a 3d Context.
	 * @param {number} w - Width of the game view.
	 * @param {number} h - Height of the game view.
	 * @param {string} divName - The Id of the Div to use as canvas.
	 * @param {boolean} autoScale - Whether the canvas should auto scale ornaw.
	...
	/**
	 * @desc Default initializer for 2d Context.
	 * @param {number} w - Width of the game view.
	 * @param {number} h - Height of the game view.
	 * @param {string} divName - The Id of the Div to use as canvas.
	 * @param {boolean} autoScale - Whether the canvas should auto scale ornaw.
...
/**
 * @desc Container for Debug related functions / variables.
 */
Lemonade.Debug = {
	/**
	 * @desc The current debug mode for the engine. Determines the types of messages that should be shown / logged.
	 */
	DebugMode: DEBUG_NONE,
	/**
	 * @desc Log a message to console / debug log with specified importance.
	 * @param {string} output - Message to log
	 * @param {number} importance - Level of importance for the message.
	 */
	...
	/**
	 * @desc Set the current debug mode of the engine.
	 * @param {number} newDebugMode - Debug mode to set. Will log any messages with specified importance.
	 */
...
/**
 * @desc Error related functions.
 */
Lemonade.Error = {
	/**
	 * @desc Used to keep track if an error is triggered.
	 */
	errorTriggered: false,
	/**
	 * @desc Trigger an error with description and code. (Code signifies a general description)
	 * @param {string} description - Description of what caused the error.
	 * @param {number} code - Code for the error. Signifies a general description.
	 */
...
/**
 * @desc Constructor for the Image object.
 * @param {string} iKey - Key of the image (used to identify)
 * @param {string} iFile - File name / location of the image.
 * @param {number} w - Width of the source image.
 * @param {number} h - Height of the source image.
 */
...
Lemonade.run = function() {
	Lemonade.Debug.log("Lemonade Engine V" + Lemonade.version, DEBUG_NOTICE);
	setTimeout(Lemonade.dd, Lemonade.Timer.timeInc); // Timer func
	setTimeout(Lemonade.bb, 1); // Logic Loop
	setTimeout(Lemonade.cc, 1); // Tween / Logic Loop
	setTimeout(Lemonade.aa, 1); // Render loop.
};
